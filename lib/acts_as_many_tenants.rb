    ...
    def acts_as_many_tenants(association = :accounts, options = {})
      ...
      unless options[:through]
        ...
        default_scope lambda {
          if ActsAsTenant.current_tenant
            if Rails::VERSION::MAJOR >= 4
              where("EXISTS (SELECT 1 FROM #{through_reflection.table_name} WHERE #{through_reflection.table_name}.#{through_reflection.foreign_key} = #{self.table_name}.id AND #{through_reflection.table_name}.#{source_reflection.association_foreign_key} = ?)", ActsAsTenant.current_tenant.id)
            else
